"""
Slide manager - handles slide metadata CRUD and seeding.
"""
import json
...
# ====== 种子数据：全部 67 页的元数据 ======
# 顺序与原始 slideOrder 一致
# 每项: (slide_id, title, type, chapter, content_fields)
SLIDE_SEED_DATA = [
    # 1: 视频开场
    ("1",  "视频开场",          "video",   "视频开场", {"video_src": "/static/intro_video.mp4"}),
...
class SlideManager:
    ...
    def update_slide(self, db: Session, slide_id: str, title: str = None, content_json: dict = None) -> Optional[SlideInfo]:
        """更新幻灯片标题和/或内容"""
        slide = db.query(SlideMeta).filter(SlideMeta.slide_id == slide_id).first()
        if not slide:
            return None
    ...
    def get_custom_html(self, db: Session, slide_id: str) -> Optional[str]:
        """获取自定义 HTML 内容"""
        slide = db.query(SlideMeta).filter(SlideMeta.slide_id == slide_id).first()
        if not slide:
            return None
    ...
    def set_custom_html(self, db: Session, slide_id: str, html: str) -> bool:
        """保存自定义 HTML 内容"""
        slide = db.query(SlideMeta).filter(SlideMeta.slide_id == slide_id).first()
        if not slide:
            return False
        ...
        content = json.loads(slide.content_json) if slide.content_json else {}
        content["html"] = html
        slide.content_json = json.dumps(content, ensure_ascii=False)
        db.commit()
        return True
