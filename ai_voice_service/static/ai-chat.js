...
(function () {
    ...
    function init() {
        ...
        bindPresentationBridge();
        connectWebSocket();
        notifyParent('service.ready', { capability: 'ai-voice-realtime' });
    }
    ...
    function suspendService() {
        ...
        stopPlayback();
        if (ws) {
            try { ws.close(1000, 'presentation left'); } catch (_) {}
            ws = null;
        }
    }
    ...
    function bindPresentationBridge() {
        window.addEventListener('message', function (event) {
            if (!parentOrigin || event.origin !== parentOrigin || event.source !== window.parent) return;
            var message = event.data || {};
            if (message.source !== 'xxh-presentation' || message.version !== 1) return;
            if (message.type === 'presentation.enter' || message.type === 'presentation.resume') {
                resumeService();
            } else if (message.type === 'presentation.leave') {
                suspendService();
            } else if (message.type === 'presentation.pause') {
                stopRecording(false);
                stopPlayback();
            } else if (message.type === 'ai.sendText') {
                var text = message.payload && message.payload.text;
                if (text) window.AIChat.sendMessage(text);
            }
        });
        window.addEventListener('beforeunload', suspendService);
    }
